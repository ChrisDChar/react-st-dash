...
export default function TeacherSingle() {
  ...
  const handleEdit = () => {
    alert(`Edit teacher ${id} - Modal coming soon!`);
  };
  ...
  const displayGender = typeof teacher.gender === 'string' ? teacher.gender.toLowerCase() : (teacher.gender ? 'male' : 'female');
  const normalizedRating = normalizeRating(teacher.rating);
  const ratingPercentage = Math.min((normalizedRating / 5) * 100, 100);

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm">
        <Link to="/teachers" className="text-blue-600 dark:text-blue-400 hover:underline">
          Teachers
        </Link>
        <span className="text-gray-400">/</span>
        <span className="text-gray-600 dark:text-gray-400">{teacher.name}</span>
      </div>
          ...
          <div className="flex flex-col items-center text-center">
            <div className="relative h-32 w-32 mb-4">
              {teacher.avatar ? (
                ...
                <div className="w-full h-full rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-4xl font-bold ring-4 ring-blue-100 dark:ring-blue-900">
                  {getInitials(teacher.name)}
                </div>
              )}
            </div>

            <h2 className="text-gray-900 dark:text-white text-xl font-bold mb-2">{teacher.name}</h2>

            <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 text-xs font-medium rounded mb-4">
            ...
            <div className="w-full space-y-4 mb-6">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-gray-900 dark:text-white">{teacher.age || 'N/A'} years</span>
              </div>

              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-600 dark:text-gray-400">Experience</span>
                <span className="text-gray-900 dark:text-white">{teacher.experience || 'N/A'} years</span>
              </div>

              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-600 dark:text-gray-400">Gender</span>
                <span className="text-gray-900 dark:text-white capitalize">{displayGender}</span>
              </div>
            ...
            <div className="p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white truncate">{teacher.email || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">{teacher.twitter || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white truncate">{teacher.linkedin || 'N/A'}</p>
                </div>
              </div>
            </div>
          </div>
